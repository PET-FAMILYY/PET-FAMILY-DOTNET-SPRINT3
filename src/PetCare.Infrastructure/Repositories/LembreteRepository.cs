...
public class LembreteRepository : Repository<Lembrete>, ILembreteRepository
{
    ...
    public LembreteRepository(PetCareContext context, IRepository<Pet> petRepository)
        : base(context)
    {
    ...
    public async Task<IEnumerable<Lembrete>> GetByPetIdAsync(int petId)
        => await DbSet.AsNoTracking().Where(l => l.PetId == petId).ToListAsync();

    public override async Task<Lembrete> AddAsync(Lembrete entity)
    {
        await ValidarPetExistenteAsync(entity.PetId);
        return await base.AddAsync(entity);
    }

    public override async Task UpdateAsync(Lembrete entity)
    {
        await ValidarPetExistenteAsync(entity.PetId);
